...
# Modifications Copyright 2026 Matteo Ugliotti, Mateo Reynoso
#
...
def spectral_curl_2d(grid: grids.Grid, velocity_hat):
...
def get_filter_fn( Delta, grid: grids.Grid ):
    
    # Calculate radial frequencies
    ks = grid.rfft_mesh()
    k2mag = sum( k**2 for k in ks)
    R = jnp.sqrt(k2mag)*2*jnp.pi
...
def get_LCR_map( Delta, grid: grids.Grid ):
    ...
    # Create the filter kernel
    G = jnp.exp(- (R**2) * (Delta**2) / 24.0)
    fft_axes = tuple(range(G.ndim))
    # Apply the filter
    def aux_filter_fn(state: spectral_types):
       state_hat = jnp.fft.rfftn( state, axes = fft_axes )
       filtered_state_hat = state_hat * G[(...,) + (None,) * (state.ndim - G.ndim)]
       return jnp.fft.irfftn( filtered_state_hat, axes = fft_axes )


    def bracket_operation( u, v ):
       
      # {u,v} = <u,v> - <u><v>
      u_bar = aux_filter_fn( u )
      v_bar = aux_filter_fn( v )
      uv = jnp.einsum('...k,...l->...kl', u, v)
      uv_squared_bar = aux_filter_fn( uv )

      u_bar_x_v_bar = jnp.einsum('...k,...l->...kl', u_bar, v_bar)

      return uv_squared_bar - u_bar_x_v_bar
    
    def L_map( state ):
       
      # L = < \bar u \bar u> - <\bar u> <\bar u >
      u_bar = aux_filter_fn( state )

      L = bracket_operation( u_bar,u_bar )
      return L
    ...
    def C_map( state ):
      ...
      C = bracket_operation( u_bar, u_prime ) + bracket_operation( u_prime,u_bar )
      return C
    ...
    def R_map( state ):
      ...
      R = bracket_operation( u_prime, u_prime )
      return R
    
    return L_map, C_map, R_map


def up_res_3d(x: jnp.ndarray, new_shape: Tuple[int, int, int]) -> jnp.ndarray:
    """
    Upsample a 3D real input x from its current shape (Nx, Ny, Nz) or 
    (Nx, Ny, Nz, Batch) to (Mx, My, Mz) (or batched equivalent) 
    by zero-padding in Fourier space.
    
    Args:
        x: Input array of shape (Nx, Ny, Nz) or (Nx, Ny, Nz, Batch).
        new_shape: Target spatial shape (Mx, My, Mz).
        
    Returns:
        Upsampled array of shape (Mx, My, Mz) or (Mx, My, Mz, Batch).
    """
    # 1. Determine dimensions and handle batching
    shape = x.shape
    ndim = len(shape)

    # Spatial dimensions are always the first three now
    if ndim not in (3, 4):
        raise ValueError(f"Input must be 3D or 4D (channel-last batch), got shape {shape}")
    
    # FFT is always performed on the first three axes (spatial dims)
    fft_axes = (0, 1, 2)

    # 2. Forward FFT (Real-to-Complex) or Input Handling
    # We check this early to correctly infer nx, ny, nz
    if not jnp.isrealobj(x):
        x_hat = x
        # If input is complex, it's already in Fourier space (rfft format).
        # Shape is (Nx, Ny, Nz//2 + 1, [Batch])
        nx, ny = shape[0], shape[1]
        nz_rfft = shape[2]
        # We assume the original spatial Z dimension was even, as per restrictions.
        # rfft size = Nz // 2 + 1  =>  Nz = (rfft size - 1) * 2
        nz = (nz_rfft - 1) * 2
    else:
        x_hat = jnp.fft.rfftn(x, axes=fft_axes)
        nx, ny, nz = shape[:3]

    mx, my, mz = new_shape
    
    # 3. Validation
    if not (mx >= nx and my >= ny and mz >= nz):
        raise ValueError("New shape must be larger than or equal to input shape.")
    
    if not all(d % 2 == 0 for d in (nx, ny, nz, mx, my, mz)):
        raise ValueError("Only even dimensions are supported to ensure symmetric padding.")
    
    # 4. Scaling factor (Volume ratio) to preserve signal magnitude
    factor = (mx * my * mz) / (nx * ny * nz)

    # 5. Prepare Padded Container
    mz_rfft = mz // 2 + 1
    
    if ndim == 4:
        # (Mx, My, Mz_half, Batch)
        padded_shape = (mx, my, mz_rfft, shape[3])
    else:
        # (Mx, My, Mz_half)
        padded_shape = (mx, my, mz_rfft)
        
    x_hat_padded = jnp.zeros(padded_shape, dtype=x_hat.dtype)

    # 6. Determine Frequency Cutoffs
    cx = nx // 2
    cy = ny // 2
    cz = nz // 2 + 1

    # 7. Copy Frequencies (Quadrant Stitching)
    # Since spatial dims are 0, 1, 2 for both cases, we can slice directly.
    # The trailing batch dimension (if present) is automatically included 
    # when we don't specify an index for it.

    # Quadrant 1: Top-Left (Pos X, Pos Y)
    x_hat_padded = x_hat_padded.at[:cx, :cy, :cz].set(x_hat[:cx, :cy, :cz])
    
    # Quadrant 2: Top-Right (Pos X, Neg Y)
    x_hat_padded = x_hat_padded.at[:cx, -cy:, :cz].set(x_hat[:cx, -cy:, :cz])
    
    # Quadrant 3: Bottom-Left (Neg X, Pos Y)
    x_hat_padded = x_hat_padded.at[-cx:, :cy, :cz].set(x_hat[-cx:, :cy, :cz])
    
    # Quadrant 4: Bottom-Right (Neg X, Neg Y)
    x_hat_padded = x_hat_padded.at[-cx:, -cy:, :cz].set(x_hat[-cx:, -cy:, :cz])

    # 8. Inverse FFT
    x_upsampled = jnp.fft.irfftn(x_hat_padded * factor, s=new_shape, axes=fft_axes)

    return x_upsampled


def get_flux_map( grid: grids.Grid ):
    
    # Calculate radial frequencies
    ks = grid.rfft_mesh()
    k2mag = sum( k**2 for k in ks)
    R = jnp.sqrt(k2mag)*2*PI
    # Create the filter kernel
    fft_axes = tuple(range(R.ndim))
    # Apply the filter
    Dz = 2j*PI*jnp.stack(ks,axis=-1)
    Lap = sum( (2j*PI*k)**2 for k in ks)

    def vorticity(state:spectral_types):
      state_hat = jnp.fft.rfft2(state, axes = fft_axes)
      omega_hat = Dz[...,0]*state_hat[...,1] - Dz[...,1]*state_hat[...,0]
      return jnp.fft.irfft2(omega_hat, axes = fft_axes)

    def sym_gradient(state:spectral_types):
       # Symmetrize gradient
       state_hat = jnp.fft.rfft2( state, axes = fft_axes )
       grad_u = jnp.einsum('...k,...l->...kl', Dz, state_hat)/2
       grad_u += jnp.einsum('...k,...l->...lk', Dz, state_hat)/2
       return jnp.fft.irfft2( grad_u, axes = fft_axes )

    def aux_filter_fn(state: spectral_types, G):
      state_hat = jnp.fft.rfft2( state, axes = fft_axes )
      filtered_state_hat = state_hat * G[(...,) + (None,) * (state.ndim - G.ndim)]
      return jnp.fft.irfft2( filtered_state_hat, axes = fft_axes )

    def bracket_operation( u, v, G ):
      
      # {u,v} = <u,v> - <u><v>
      u_bar = aux_filter_fn( u, G )
      v_bar = aux_filter_fn( v, G )
      uv = jnp.einsum('...k,...l->...kl', u, v)
      uv_squared_bar = aux_filter_fn( uv, G )

      u_bar_x_v_bar = jnp.einsum('...k,...l->...kl', u_bar, v_bar)

      return uv_squared_bar - u_bar_x_v_bar
    
    def scalar_bracket_operation( u, v, G ):
      
      # {u,v} = <u,v> - <u><v>
      u_bar = aux_filter_fn( u, G )
      v_bar = aux_filter_fn( v, G )
      uv = jnp.einsum('...,...l->...l', u, v)
      uv_squared_bar = aux_filter_fn( uv, G )

      u_bar_x_v_bar = jnp.einsum('...,...l->...l', u_bar, v_bar)

      return uv_squared_bar - u_bar_x_v_bar
    
    def sgs_tensor_fn( state, delta ):
      # tau = < u u> - < u > < u >
      G = jnp.exp(- (R**2) * (delta**2) / 24.0)
      #u_bar = aux_filter_fn( state, G )
      sgs = bracket_operation( state,state, G )
      return sgs
    
    def sgs_tensor( state, G ):
      # tau = < u u> - < u > < u >
      #u_bar = aux_filter_fn( state, G )
      sgs = bracket_operation( state,state, G )
      return sgs

    def energy_flux_fn( state, delta ):
      # Compute tr( S \tau )      
        G = jnp.exp(- (R**2) * (delta**2) / 24.0)
        tau = sgs_tensor( state, G )
        S = sym_gradient(aux_filter_fn( state, G ))
        flux = jnp.einsum('...ij,...ji->...', tau,S)
        flux = tau[...,1,1]*S[...,1,1]+tau[...,0,0]*S[...,0,0]
        flux += 2*tau[...,1,0]*S[...,1,0]
        return  - flux
    #tau[...,0,0],tau[...,1,1],tau[...,0,1]#
    
    def enstrophy_flux_fn( state, delta ):
      # Compute tr( S \tau )      
        G = jnp.exp(- (R**2) * (delta**2) / 24.0)
        omega = vorticity(state)

        vartheta = scalar_bracket_operation( omega, state, G )

        omega_bar = G*jnp.fft.rfft2(omega,axes=fft_axes)
        grad_omega = jnp.stack([jnp.fft.irfft2(Dz[...,0]*omega_bar,axes=fft_axes),
                                jnp.fft.irfft2(Dz[...,1]*omega_bar,axes=fft_axes)],axis=-1)
        # tau = sgs_tensor( state, G )
        # tau_hat = jnp.fft.rfft2(tau,axes=fft_axes)
        # L_tau_hat = tau_hat * Lap[(...,) + (None,) * (tau.ndim - Lap.ndim)]
        # L_tau= jnp.fft.irfft2( L_tau_hat ,axes=fft_axes)
        # S = sym_gradient(aux_filter_fn( state, G ))
        flux = jnp.einsum('...i,...i->...', grad_omega,vartheta)
        return -flux

    return sgs_tensor_fn, energy_flux_fn, enstrophy_flux_fn

def vorticity_to_streamfunction(
    grid: grids.Grid,
) -> Callable[[Array], Array]:
    """
    Returns a function that maps spectral vorticity (omega_hat)
    to spectral streamfunction (psi_hat).

    Args:
        grid: jax_cfd Grid

    Returns:
        Function: omega_hat -> psi_hat
    """
    kx, ky = grid.rfft_mesh()
    two_pi_i = 2j * jnp.pi

    laplacian = (two_pi_i ** 2) * (kx**2 + ky**2)
    inv_lap =  1 / laplacian
    inv_lap = inv_lap.at[0, 0].set(0)  # avoid division by zero

    def omega_hat_to_psi_hat(omega_hat: Array) -> Array:
