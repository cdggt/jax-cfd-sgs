# Copyright 2026 Mateo Reynoso, Matteo Ugliotti
#
...
from jax_cfd.spectral.utils import fourier_filter
...
class Similarity(time_stepping.ImplicitExplicitODE):
    ...
    def __post_init__(self):
        ...
        if self.forcing_fn is not None:
            ...
            self.f_hat = jnp.stack(spectral_utils.vorticity_to_velocity(self.grid)(curl_f_hat),axis=-1)

        
        self.fourier_filter = spectral_utils.get_filter_fn(Delta=self.Delta,grid=self.grid)
    ...
    def explicit_terms(self, state):
        ...
        U_bar = jnp.fft.irfft2( self.fourier_filter(U_hat), axes = (0,1) )
        UbUb = jnp.fft.rfft2( jnp.einsum( '...i,...j->...ij',U,U), axes=(0,1) )

        UbUb_bar = jnp.fft.irfft2(self.fourier_filter( UbUb),axes=(0,1))

        tau_xx = UbUb_bar[...,0,0] - U_bar[..., 0] * U_bar[..., 0]
        tau_xy = UbUb_bar[...,0,1] - U_bar[..., 0] * U_bar[..., 1]
        tau_yy = UbUb_bar[...,1,1] - U_bar[..., 1] * U_bar[..., 1]
        ...
        if self.forcing_fn is not None:
            # Forcing is uncurled in the class constructor
            terms_les += self.f_hat
...
@dataclasses.dataclass
class Dynamic_Smagorinsky(time_stepping.ImplicitExplicitODE):
    viscosity: float
    grid: grids.Grid
    Delta: float
    alpha: float = 2
    ...
    def __post_init__(self):
        ...
        self.inv_laplace = jnp.where(self.laplace == 0, 0.0, 1 / self.laplace)
        self.linear_term = self.viscosity * self.laplace - self.drag
        self.DeltaTest = self.Delta * self.alpha
        if self.forcing_fn is not None:
            self._forcing_fn_with_grid = self.forcing_fn(self.grid)
            curl_f_hat = self._forcing_fn_with_grid()
            self.f_hat = jnp.stack(spectral_utils.vorticity_to_velocity(self.grid)(curl_f_hat),axis=-1)
    ...
    def explicit_terms(self, state):
        ...
        Sxx = dUdx[..., 0]
        Sxy = 0.5 * (dUdx[..., 1] + dUdy[..., 0])
        Syy = dUdy[..., 1]
        I1 = jnp.sqrt(2.0 * (Sxx**2 + Syy**2 + 2.0 * Sxy**2))

        TSxx = fourier_filter(Sxx, self.DeltaTest)
        TSxy = fourier_filter(Sxy, self.DeltaTest)
        TSyy = fourier_filter(Syy, self.DeltaTest)
        TsI1 = jnp.sqrt(2.0 * (TSxx**2 + TSyy**2 + 2.0 * TSxy**2))

        Fu_T = fourier_filter(U[..., 0], self.DeltaTest)
        Fv_T = fourier_filter(U[..., 1], self.DeltaTest)
        LT_xx = fourier_filter(U[..., 0]**2, self.DeltaTest) - Fu_T * Fu_T
        LT_xy = fourier_filter(U[..., 0] * U[..., 1], self.DeltaTest) - Fu_T * Fv_T
        LT_yy = fourier_filter(U[..., 1]**2, self.DeltaTest) - Fv_T * Fv_T

        Fu_D = fourier_filter(U[..., 0], self.Delta)
        Fv_D = fourier_filter(U[..., 1], self.Delta)
        LΔ_xx = fourier_filter(U[..., 0]**2, self.Delta) - Fu_D * Fu_D
        LΔ_xy = fourier_filter(U[..., 0] * U[..., 1], self.Delta) - Fu_D * Fv_D
        LΔ_yy = fourier_filter(U[..., 1]**2, self.Delta) - Fv_D * Fv_D

        Mxx = 2.0 * self.Delta**2 * ((self.alpha**2) * TsI1 * TSxx - fourier_filter(I1 * Sxx, self.DeltaTest))
        Mxy = 2.0 * self.Delta**2 * ((self.alpha**2) * TsI1 * TSxy - fourier_filter(I1 * Sxy, self.DeltaTest))
        Myy = 2.0 * self.Delta**2 * ((self.alpha**2) * TsI1 * TSyy - fourier_filter(I1 * Syy, self.DeltaTest))

        num = LT_xx * Mxx + LT_yy * Myy + 2.0 * LT_xy * Mxy
        denom = Mxx * Mxx + Myy * Myy + 2.0 * Mxy * Mxy
        Cs2 = num / denom

        # --- Eddy viscosity ---
        nu_t = (self.Delta**2) * Cs2 * I1
        if self.enforce_nu_pos:
            nu_t = jnp.maximum(nu_t, 0.0)
...
@dataclasses.dataclass
class Dynamic_Mixed(time_stepping.ImplicitExplicitODE):
    """SGS: τ_ij = """
    viscosity: float
    grid: grids.Grid
    Delta: float
    ...
    def __post_init__(self):
        ...
        self.inv_laplace = jnp.where(self.laplace == 0, 0.0, 1 / self.laplace)
        self.linear_term = self.viscosity * self.laplace - self.drag
        self.DeltaTest = self.alpha*self.Delta
        if self.forcing_fn is not None:
            self._forcing_fn_with_grid = self.forcing_fn(self.grid)
            curl_f_hat = self._forcing_fn_with_grid()
            self.f_hat = jnp.stack(spectral_utils.vorticity_to_velocity(self.grid)(curl_f_hat),axis=-1)
    ...
    def explicit_terms(self, state):
        ...
        tau_xx = LΔ_xx - 2.0 * nu_t * Sxx
        tau_xy = LΔ_xy - 2.0 * nu_t * Sxy
        tau_yy = LΔ_yy - 2.0 * nu_t * Syy
...
class LES_w_TR(time_stepping.ImplicitExplicitODE):
    ...
    def __post_init__(self):
        ...
        self.linear_term = self.viscosity * self.laplace - self.drag
        #TODO, place again the linear term
        self.linear_term_R = self.viscosity * self.laplace - 0*self.drag 
        self.w_to_v = spectral_utils.vorticity_to_velocity(self.grid)
        self.filter_fn = spectral_utils.get_filter_fn( self.Delta*1.5, self.grid )
        self.U_size = self.grid.shape + (2,)
    ...
    def explicit_terms(self, state):
        ...
        U_space = jnp.fft.irfft2( U_les, axes = (0,1) )
        R_space = jnp.fft.irfft2( R_ij, axes = (0,1))
        # R is Rxx', Rxy, where Rxx' is the projection to the traceless-symmetric
        # rank-2 subspace, note Rxx' = - Ryy'
        ...
        ngmR += ngm4
        # Pass τxx',  τxy as the subgrid tensor
        ngmR = jnp.fft.rfft2( ngmR, axes = (0,1) ) + R_ij
        # Compute divergence of tau
        # \div \tau = [dτxx'dx + dτxydy, dτxydx - dτxx'dy]
        div_tau = jnp.stack( [ self.two_i_pi * self.kx * ngmR[...,0] + self.two_i_pi * self.ky * ngmR[...,1],\
                               self.two_i_pi * self.kx * ngmR[...,1] - self.two_i_pi * self.ky * ngmR[...,0]],\
                               axis = -1)
        terms_les -= div_tau
        ...
        rhs_R = - R_adv
        # Production terms of R
        # R(il Dl uj) - \deltaij/2 Rml Dl um = { 0.5*Rxy*(dudy - dvdx),  0.5*R'xx(dvdx - dudy) }
        prod_R = jnp.stack([R_space[...,1] * ( dUdy[...,0] - dUdx[...,1] ),
                            R_space[...,0] * ( dUdx[...,1] - dUdy[...,0] ),
                            ], axis=-1) # Production is two times the symmetrization
        ...
        dLUdx = jnp.fft.irfft2( self.two_i_pi * self.kx[...,None] * self.laplace[...,None] * U_les, axes = (0,1) )
        dLUdy = jnp.fft.irfft2( self.two_i_pi * self.ky[...,None] * self.laplace[...,None] * U_les, axes = (0,1) )
        # Commpute ngm expasion for  k (Rii)
        ...
        # Compute invariants
        I1 = 4*dUdx[...,0]**2 + (dUdx[...,1] + dUdy[...,0])**2  # 2 Sij Sij
        I2 = (dUdy[...,0] - dUdx[...,1])**2                     # 2 Oij Oij
        I = jnp.sqrt(I1[...,None] + I2[...,None])
        # Linear term
        ...
        nu_eddy *= self.sigma_sq**3/(self.Delta)/4

        
        self.nu_R = I.mean() * self.sigma_sq**3
        self.nu_eddy = nu_eddy.mean()
        #terms_les += jnp.fft.rfft2(nu_eddy.mean() * LU_space, axes = (0,1) )   
    ...
    def implicit_terms(self, state):
        ...
        order_R = 3
        return (self.linear_term[...,None] + self.nu_eddy*self.laplace[...,None]**order) * U_les ,\
              ( self.linear_term_R[...,None] + self.nu_R*self.laplace[...,None]**order_R ) * R_ij
    #1e3*self.nu_eddy*self.laplace[...,None]**order
    def implicit_solve(self, y_hat, dt):
        ...
        # + nu_eddy*self.laplace[...,None]**order
        return 1 / (1 - dt * (self.linear_term[...,None] + self.nu_eddy*self.laplace[...,None]**order)) * U_les, \
          1 / (1 - dt * (self.linear_term_R[...,None] + self.nu_R*self.laplace[...,None]**order_R ) ) * R_ij  
...
@dataclasses.dataclass
class LCR(time_stepping.ImplicitExplicitODE):
    viscosity: float
    grid: grids.Grid
    Delta: float
    drag: float = 0.0
    ...
    def __post_init__(self):
        ...
        if self.k_filter is None:
            ...
            self.filter_ = spectral_utils.brick_wall_filter_2d(self.grid, self.k_filter)
        self.filter_R = spectral_utils.brick_wall_filter_2d(self.grid, 1/2)
        self.linear_term = self.viscosity * self.laplace - self.drag
        self.w_to_v = spectral_utils.vorticity_to_velocity(self.grid)
        self.filter_fn = spectral_utils.get_filter_fn(self.Delta, self.grid)
        self.U_size = self.grid.shape + (2,)
        if self.forcing_fn is not None:
            self._forcing_fn_with_grid = self.forcing_fn(self.grid)
            curl_f_hat = self._forcing_fn_with_grid()
    ...
    def explicit_terms(self, state):
        U_les, R_ij = state              # both in Fourier
        U_space = jnp.fft.irfft2(U_les, axes=(0, 1))
        R_space = jnp.fft.irfft2(R_ij,  axes=(0, 1))
        ...
        ngm4yy = dUdxdx[..., 1]**2 + dUdydy[..., 1]**2 + 2*dUdxdy[..., 1]**2
        ngm4xy = dUdxdx[..., 0]*dUdxdx[..., 1] + 2*dUdxdy[..., 0]*dUdxdy[..., 1] + dUdydy[..., 0]*dUdydy[..., 1]
        ngmR  += self.Delta**4/288 * jnp.stack([ngm4xx, ngm4yy, ngm4xy], axis=-1)
 
        tr = 0.5 * (ngmR[..., 0] + ngmR[..., 1])
        ngmR = ngmR.at[..., 0].add(-tr).at[..., 1].add(-tr)
 
        ngmR_hat = self.filter_[..., None] * jnp.fft.rfft2(ngmR, axes=(0, 1))
        tau_hat = ngmR_hat + R_ij
 
        div_tau = jnp.stack(
            [ self.two_i_pi * self.kx * tau_hat[..., 0] + self.two_i_pi * self.ky * tau_hat[..., 2],
              self.two_i_pi * self.kx * tau_hat[..., 2] + self.two_i_pi * self.ky * tau_hat[..., 1] ],
            axis=-1
        )
        terms_les -= self.filter_[..., None] * div_tau
 
        UxR = U_space[..., 0:1] * R_space
        UyR = U_space[..., 1:2] * R_space
        UxR_hat = self.filter_[..., None] * jnp.fft.rfft2(UxR, axes=(0, 1))
        UyR_hat = self.filter_[..., None] * jnp.fft.rfft2(UyR, axes=(0, 1))
        rhs_R = - jnp.fft.irfft2(
            self.two_i_pi*self.kx[..., None]*UxR_hat + self.two_i_pi*self.ky[..., None]*UyR_hat,
            axes=(0, 1)
        )
 
        prod_R = jnp.stack(
            [ 2*dUdx[..., 0]*R_space[..., 0] + 2*dUdy[..., 0]*R_space[..., 2],
              2*dUdx[..., 1]*R_space[..., 2] + 2*dUdy[..., 1]*R_space[..., 1],
                dUdx[..., 1]*R_space[..., 0] +   dUdy[..., 0]*R_space[..., 1] ],
            axis=-1
        )
        ...
        I1 = 4*dUdx[..., 0]**2 + (dUdx[..., 1] + dUdy[..., 0])**2
        I2 = (dUdy[..., 0] - dUdx[..., 1])**2
        I  = (jnp.sqrt(I1 + I2))[...,None]
        dLUdx = jnp.fft.irfft2(self.two_i_pi * self.kx[..., None] * self.laplace[..., None] * U_les, axes=(0, 1))
        dLUdy = jnp.fft.irfft2(self.two_i_pi * self.ky[..., None] * self.laplace[..., None] * U_les, axes=(0, 1))
        traceR = self.Delta**6/6912 * (
            (dLUdx[..., 0]**2 + dLUdy[..., 0]**2) +
            (dLUdx[..., 1]**2 + dLUdy[..., 1]**2)
        )
        traceR = jnp.fft.irfft2(self.filter_*(jnp.fft.rfft2(traceR, axes=(0, 1))), axes=(0, 1))
        F_ij = jnp.stack([ traceR* dUdx[...,0],traceR*dUdy[...,1],traceR*( dUdx[...,1]+dUdy[...,0] )/2], axis=-1)
 
        nu_eddy_field = (R_space[..., 0]**2+ R_space[..., 1]**2 + 2*R_space[..., 2]**2)**(1/4)*self.Delta**5/(12**3)
        I1 = 4*dUdx[..., 0]**2 + (dUdx[..., 1] + dUdy[..., 0])**2
        I2 = (dUdy[..., 0] - dUdx[..., 1])**2
        I_scalar = jnp.sqrt(I1 + I2)            # (Nx,Ny)
        I = I_scalar[..., None]                 # (Nx,Ny,1) for -C1 I R
 
        # ---------- D1 term: Δ^5/1728 (R_ij R_ij)^{1/4} ∇^6 u ----------
        R_sq = (
            R_space[..., 0]**2 +
            R_space[..., 1]**2 +
            2.0*R_space[..., 2]**2
        )                                       # (Nx,Ny)
        nu_6 = (R_sq**0.25) * (self.Delta**5 / 12**3)   # 1728 = 12^3
 
        # ∇^6 u in physical space
        lap3U_hat = (self.laplace**3)[..., None] * U_les   # (Nx,Ny_fft,2)
        lap3U     = jnp.fft.irfft2(lap3U_hat, axes=(0, 1)) # (Nx,Ny,2)
 
        D1_real = nu_6[..., None] * lap3U                  # (Nx,Ny,2)
        D1_hat  = jnp.fft.rfft2(D1_real, axes=(0, 1))
        terms_les += 0.1*D1_hat
 
        rhs_R += F_ij - self.C1 * I * R_space
 
        # ---------- D2 term: Δ^6/1728 I ∇^6 R_ij ----------
        lap3R_hat = (self.laplace**3)[..., None] * R_ij       # (Nx,Ny_fft,3)
        lap3R     = jnp.fft.irfft2(lap3R_hat, axes=(0, 1))    # (Nx,Ny,3)
 
        nu_R_real = (self.Delta**6 / 12**3) * I_scalar        # (Nx,Ny)
        D2_real   = nu_R_real.mean() * lap3R              # (Nx,Ny,3)
 
        rhs_R += D2_real
 
 
        Rxx, Ryy = rhs_R[..., 0], rhs_R[..., 1]
        s = 0.5 * (Rxx - Ryy)
        rhs_R = rhs_R.at[..., 0].set(s).at[..., 1].set(-s)
 
        rhs_R = self.filter_R[..., None] * jnp.fft.rfft2(rhs_R, axes=(0, 1))
        div_rhs = self.two_i_pi * (self.kx * terms_les[..., 0] + self.ky * terms_les[..., 1])
        leray = jnp.stack([self.two_i_pi * self.kx, self.two_i_pi * self.ky], axis=-1)
        leray *= div_rhs[..., None] * self.inv_laplace[..., None]
        terms_les -= leray
        ...
        # optional post-smoothing (still your filter)
        if self.smooth:
            terms_les *= self.filter_[..., None]
            rhs_R    *= self.filter_[..., None]
            rhs_R    *= self.filter_R[..., None]
 
        return terms_les, rhs_R
 
    def implicit_terms(self, state):
        U_les, R_ij = state
        return (self.linear_term[..., None]) * U_les, (self.linear_term[..., None]) * R_ij
 
    def implicit_solve(self, y_hat, dt):
        U_les, R_ij = y_hat
        denU = 1 - dt * (self.linear_term[..., None])
        return U_les / denU, R_ij / denU
