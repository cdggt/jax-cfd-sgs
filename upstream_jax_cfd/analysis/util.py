...
@jax.jit
def power_spectrum_3d_jax(field: jnp.ndarray, kx: jnp.ndarray, ky: jnp.ndarray, kz: jnp.ndarray, nbins: int = None):
    """
    Compute the isotropic 3D power spectrum of a scalar or vector field using JAX.

    Args:
        field (jnp.ndarray or tuple/list of jnp.ndarray): Scalar (Nx, Ny, Nz) or vector field (n_comp, Nx, Ny, Nz)
        kx, ky, kz (jnp.ndarray): 3D wavenumber grids matching the FFT output shape
        nbins (int, optional): Number of bins for isotropic spectrum. Defaults to Nx//2
    ...
    # --- Process scalar or vector field ---
    if isinstance(field, (tuple, list)):
        # Vector field: stack along component axis
        field_array = jnp.stack(field, axis=0)
        Nvol = jnp.prod(jnp.array(field_array.shape[1:]))
        field_array = field_array / Nvol
        fft_result = jnp.fft.rfftn(field_array, axes=(1,2,3))
        power = jnp.sum(jnp.abs(fft_result)**2, axis=0)  # sum over components
    else:
        field = field / field.size
        fft_result = jnp.fft.rfftn(field)
        power = jnp.abs(fft_result)**2

    # --- Apply RFFT symmetry correction along the last axis ---
    Nz_half = power.shape[-1]
    jax.debug.print("Input x: {x}",x = Nz_half)
    correction = jnp.ones(Nz_half)
    correction = correction.at[1:-1].set(2.0)
    power = power * correction[None, None, :]  # broadcast over x and y

    # --- Compute wavenumber magnitude ---
    k_mag = jnp.sqrt((2*jnp.pi*kx)**2 + (2*jnp.pi*ky)**2 + (2*jnp.pi*kz)**2)
    k_flat = k_mag.flatten()
    power_flat = power.flatten()

    # --- Define bins ---
    if nbins is None:
        nbins = power.shape[0] // 2
    k_bins = jnp.linspace(0.0, k_flat.max(), nbins + 1)
    k_bin_centers = 0.5 * (k_bins[:-1] + k_bins[1:])
    ...
    bin_idx = digitize(k_flat, k_bins)
    in_bounds = (bin_idx >= 0) & (bin_idx < nbins)
    safe_bin_idx = jnp.where(in_bounds, bin_idx, 0)
    safe_power_flat = jnp.where(in_bounds, power_flat, 0.0)

    # --- Bin accumulation ---
    Pk_sum = jnp.bincount(safe_bin_idx, weights=safe_power_flat, length=nbins)
